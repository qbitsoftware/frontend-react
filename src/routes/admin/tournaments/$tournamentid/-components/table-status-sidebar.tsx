...
const TableStatusSidebar = () => {
    ...
    const tableStatuses = useMemo((): TableStatus[] => {
        ...
        return tournamentTables.data.map(table => {
            ...
            return {
                ...
                match: match?.p1?.name && match?.p2?.name ? {
                    participant1: match.p1.name,
                    participant2: match.p2.name,
                    groupName: tournamentTableId ? groupsMap.get(tournamentTableId) || "" : "",
                } : undefined
            };
        });
    ...
    const handleRowClick = (table: TableStatus) => {
        if (table.tournament_table_id) {
            router.navigate({ to: `/admin/tournaments/${tournamentid}/grupid/${table.tournament_table_id}/mangud` });
        }
    }
            ...
            <div className="flex flex-col overflow-y-auto w-full h-full">
                {tableStatuses.map((table) => (
                    <div
                        ...
                        key={table.id}
                        className={cn("w-full flex items-center gap-4 justify-between h-12 px-2 overflow-hidden border-b", table.tournament_table_id && "cursor-pointer")}>
                        <h3 className="text-lg font-semibold">
                            {table.number}
                        </h3>
                        {table.match && (
