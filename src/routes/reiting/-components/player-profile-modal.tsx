...
export const PlayerProfileModal = ({ isOpen, onClose, user }: PlayerProfileModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[90vw] md:max-w-6xl h-[90vh] md:h-[700px] overflow-y-auto py-0 px-0 bg-white rounded-2xl shadow-2xl border-0">
        <DialogClose className="absolute right-6 top-6 p-3 rounded-full bg-white/80 backdrop-blur-sm hover:bg-white shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-[#4C97F1]/20 transition-all duration-300 z-10">
          <X className="h-5 w-5 text-gray-600 hover:text-gray-800" />
          <span className="sr-only">Close</span>
        </DialogClose>
        <div className="flex flex-col md:flex-row h-full">
          <div className="flex flex-col items-center space-y-6 w-full md:w-1/3 p-8 bg-gradient-to-br from-[#4C97F1]/5 to-blue-50/50">
            <Avatar className="w-32 h-32 md:w-48 md:h-48 shadow-2xl ring-4 ring-white">
            ...
            <div className="flex flex-col items-center gap-3">
              <div className="bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-md">
                <p className="text-[#4C97F1] font-semibold text-sm">{user.club_name}</p>
              </div>
